...
class KnowledgeBase:
    # the knowledge bsae with a Dict of Symbols and Compound Sentences
    def __init__(self):
        self.Sentences = {}
        self.SymbolTable = []
        self.modelList = []
    ...
    def buildModels(self, symbolList, model):
        # build up the truth-table and set entries for every model
        if len(symbolList) == 0:
    ...
    def resolution(self, model):
        # Extract the model whose KB has True value
        isSat = True
            ...
            if type(self.Sentences[key]) is CompoundSentence:
                if self.Sentences[key].KB_state:
                    if not (self.Sentences[key].checkSatisfied(model)):
                        isSat = False
        if isSat and model is not None:
            return True

    def calcKB(self, model=None):
        # Judge the value of each sentence
        isSat = True
        for key in sorted(self.Sentences.keys()):
            if type(self.Sentences[key]) is CompoundSentence:
                if self.Sentences[key].KB_state:
    ...
    def checkEntailment(self, TargetName):
        # Check if the KB entails target
        isEntailed = True
        for model in self.modelList:
            ...
            for key in sorted(self.Sentences.keys()):
                if (type(self.Sentences[key]) is CompoundSentence) and (self.Sentences[key].KB_state) and (
                    ...
                    kb_isSat = False
                    break
            if kb_isSat and not model.symbolMap[TargetName]:
                isEntailed = False
        return isEntailed
...
print("Prove KB and not", KB.Target, "is unsatisfiable")
...
print("Result of resolution: ", KB.Target, 'is', KB.checkEntailment(KB.Target))
